...
class ScheduledEmail(db.Model):
    """Scheduled emails generated by AI"""
    ...
    body = db.Column(db.Text, nullable=False)  # HTML formatted
    
    # Status
    status = db.Column(db.String(50), default='pending')  # pending, sent, cancelled
    scheduled_at = db.Column(db.DateTime, nullable=False)  # When to send (6 hours after deal)
    
    # Timestamps
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    sent_at = db.Column(db.DateTime)  # When actually sent
    cancelled_at = db.Column(db.DateTime)  # When cancelled
    
    # Relationships
    user = db.relationship('User', backref='scheduled_emails')
    deal = db.relationship('Deal', backref='scheduled_emails')
    
    __table_args__ = (
        db.Index('idx_scheduled_user_status', 'user_id', 'status'),
        db.Index('idx_scheduled_thread', 'thread_id'),
        db.Index('idx_scheduled_at', 'scheduled_at'),
    )
    
    def __repr__(self):
        return f'<ScheduledEmail {self.status} to {self.to_email} scheduled {self.scheduled_at}>'
